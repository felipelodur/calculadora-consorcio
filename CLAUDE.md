...
- App: `flask --app app.flask_app run --debug`
...
2. **`app/`** — Flask web app. `flask_app.py` serves the API, `templates/index.html` + `static/` handle the frontend. Portuguese BR.
...
- **BCB data has two cache layers**: disk CSV cache in `data/cache/` + in-memory. The `BCBDataProvider.get()` method checks disk cache first.
...
- **Add a UI section**: edit `app/templates/index.html` and `app/static/app.js`. API endpoints in `app/flask_app.py`.
